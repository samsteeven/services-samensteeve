...
export default function SecondBrainHowIBuiltIt() {
  return (
    <article className="prose dark:prose-invert max-w-none text-ink-soft leading-relaxed font-sans text-sm md:text-base space-y-6">
      ...
      <ZoomableImage src="/blog/second-brain-ask.png" alt="The query workflow: question, vectorization, Qdrant search, context, LLM, sourced answer" />
      <p className="text-xs text-ink-soft text-center italic mb-6">The query pipeline in n8n: this is the workflow the <code>second_brain_ask</code> MCP tool calls.</p>
...
Before writing, list the information you found in my base and any gaps with the posting. Then propose the CV.`}
      ...
      />
      <p><strong>The result.</strong> A targeted CV: Kubernetes and Terraform for DevOps, Spring Boot for Java, React for the frontend, Second Brain for AI, and a section that honestly ties my experience to <strong>Green IT</strong> (cloud cost and resource control).</p>
      <p>What I found most useful wasn&apos;t the CV, though. It was the list of gaps. The AI points out the <strong>real holes in my base</strong>: projects described too quickly (React only via SIGGE, with no detail of what I did there), a project presented as a case study rather than a real mission, missing skills (Kotlin, etcd, Operator Framework). Its remark about &quot;truncated files&quot; deserved a check: it came less from my base than from my search engine. My indexer split notes <strong>in the middle of sentences</strong>, with no heading and no context, and only surfaced <strong>eight chunks per question</strong>. The content was often there, but badly split, so unreachable. Ever since I split <strong>one chunk per section</strong> and prefix it with its source, information I thought was missing now ranks at the top of the results, like <strong>my role on TribuneJustice</strong>. The brain is only worth what you put into it, and also what it manages to retrieve. It&apos;s a start, and it makes those holes visible.</p>
